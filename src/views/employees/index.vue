  ...
  methods: {
    ...
    exportData() {
      ...
      import('@/vender/Export2Excel').then(async excel => {
        // 获取员工的接口 传入两参数 页码 每页条数
        const { rows } = await getEmployeeList({ page: 1, size: this.page.total })
        const data = this.formatJson(headers, rows) // data 即要导出的结构
    ...
    formatJson(headers, rows) {
      ...
      return rows.map(item => {
        // item 是对象 { mobile: 1234, username: 'zhsan}
    ...
    showQrCode(url) {
      // url 存在情况下 弹出层
      if (url) {
        this.showCodeDialog = true // 数据更新 页面还没更新
        // 在上一次数据更新完毕，页面渲染完毕之后执行
        this.$nextTick(() => {
          QrCode.toCanvas(this.$refs.myCanvas, url) // 将地址转化为二维码链接
          // 如果转化的二维码后面信息  是一个地址的话  就会跳转到改地址
        })
      } else {
    ...
    async editRole(id) {
      // 弹出层
      this.userId = id // props 赋值 props 赋值渲染异步
